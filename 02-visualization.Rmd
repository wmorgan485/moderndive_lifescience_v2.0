...
For now, we suggest you don't worry if you don't fully understand this code. We'll see later in Chapter \@ref(wrangling) on data wrangling that this code uses the `dplyr` package for data wrangling to achieve our goal: it takes the `all_fishdata` data frame and `filter`s it to only return the rows where `Brack` is equal to `-1` (paradoxically indicating that this is true, as opposed to 0/false). Recall from Section \@ref(code) that testing for equality is specified with \index{using == instead of =} `==` and not `=`. Convince yourself that this code achieves what it is supposed to by exploring the resulting data frame by running `View(brackish_fish)`. You'll see that it has `r brack_species_rows` rows, consisting of only `r brack_species_rows` brackish fish species. 
...
Let's first unpack the graphic in Figure \@ref(fig:noalpha). Observe that a *positive relationship* exists overall between `Length` and `Weight`: as fish lengths increase, fish weights tend to also increase. Observe also the large number of points clustered near (0, 0).

Let's turn our attention to the warning message. R is alerting us to the fact that many rows were ignored due to missing information. For these rows, either the value for `Length` or `Weight` or both were missing (recorded in R as `NA`), and thus these rows were ignored in our plot.
...
Note that the last line of R code rotates and shifts the labels so that they don't overlap.

Observe in Figure \@ref(fig:habitatsbar) that demersal, benthopelagic, and reef-associated environments were the preferred habitat for the most fish species. 

Alternatively, say you had a data frame where the number of fish species preferring each habitat (the `DemersPelag` variable) was pre-counted as in Table \@ref(tab:all-fishdata-counted).
...
Lastly, another type of barplot is a \index{barplot!faceted} *faceted barplot*. Recall in Section \@ref(facets) we visualized the distribution of weight gain for the chicks on Diet 1 *split* by the Chick identifier (1-50) using facets. We apply the same principle to our barplot visualizing the frequency of `DemersPelag` split by `BodyShapeI`: instead of mapping `BodyShapeI` to `fill` we include it as the variable to create small multiples of the plot across the levels of `BodyShapeI`.
