...
```{block loa2, type="review", purl=FALSE}
\vspace{-0.15in}

**_Chapter Learning Objectives_**

*At the end of this chapter, you should be able to…*  
• Explore and present data using plots (a.k.a., charts and graphics) in R.  
• Use the grammar of graphics to create basic plots by specifying the data, its mapping and the geometric object(s).  
• Produce more complex plots by adding layers to modify labels, add facets, etc.  
• Adjust plot features such as colors and bin number by modifying function arguments (inputs).  
• Select and create the most appropriate graph to visualize your data.  
• Recognize and correct difficulties in visualizing data due to overplotting.  
• Interpret data presented in histograms and boxplots.  
• Explain the problems with using bar charts to present quantitative data and pie charts to present categorical data.  
\vspace{-0.1in}
```
...
We'll also present some variations of these plots, but with this basic repertoire of five graphics in your toolbox, you can visualize a wide array of different variable types. As we'll see, certain plots are only appropriate for categorical variables, while others are only appropriate for numerical variables. 
...
The simplest of the 5NG are *scatterplots*, \index{scatterplots} also called *bivariate plots*. They allow you to visualize the *relationship* between two numerical variables. While you may already be familiar with scatterplots, let's view them through the lens of the grammar of graphics we presented in Section \@ref(grammarofgraphics). Specifically, we will visualize the relationship between the following two numerical variables in the `all_fishdata` data frame we created in Chapter \@ref(fishbasedataframe):

1. `Length`: typical length (in cm) of a fish species on the horizontal "x" axis and
1. `Weight`: typical weight (in g) on the vertical "y" axis

for brackish fish species. (If the `all_fishdata` set is not in your Environment, rerun the `all_fishdata <- species()` command to reproduce it.)

```{r purl=FALSE, echo=FALSE}
# This redundant code is used for dynamic non-static in-line text output purposes
species_rows <- all_fishdata %>%
  nrow() %>%
  comma()
brack_species_rows <- all_fishdata %>%
  filter(Brack == 1) %>%
  nrow() %>%
  comma()
```

This requires paring down the data from all `r species_rows` fish species to only the `r brack_species_rows` *brackish* species that live in brackish (slightly salty) environments at any stage of development. We do this so our scatterplot will involve a manageable `r brack_species_rows` points, and not an overwhelmingly large number. To achieve this, we'll take the `all_fishdata` data frame, filter the rows so that only the `r brack_species_rows` rows corresponding to brackish fish species are kept, and save this in a new data frame called `brackish_fish` using the `<-` *assignment* operator: \index{operators!assignment (<-)} 
...
For now, we suggest you don't worry if you don't fully understand this code. We'll see later in Chapter \@ref(wrangling) on data wrangling that this code uses the `dplyr` package for data wrangling to achieve our goal: it takes the `all_fishdata` data frame and `filter`s it to only return the rows where `Brack` is equal to `1` (indicating that this is true, as opposed to 0/false). Recall from Section \@ref(code) that testing for equality is specified with \index{using == instead of =} `==` and not `=`. Convince yourself that this code achieves what it is supposed to by exploring the resulting data frame by running `View(brackish_fish)`. You'll see that it has `r brack_species_rows` rows, consisting of only `r brack_species_rows` brackish fish species. 

```{block lc-brackish_fish, type="learncheck", purl=FALSE}
\vspace{-0.15in}
**_Learning check_**
\vspace{-0.1in}
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  Take a look at both the `all_fishdata` and `brackish_fish` data frames by running `View(all_fishdata)` and `View(brackish_fish)`. Does the `brackish_fish` data frame seem to contain the expected rows?
...
1. The `data` as the `brackish_fish` data frame via `data = brackish_fish`.
1. The `aes`thetic \index{ggplot2!mapping} `mapping` by setting `mapping = aes(x = Length, y = Weight)`. Specifically, the variable `Length` maps to the `x` position aesthetic, while the variable `Weight` maps to the `y` position.
...
```{r noalpha, fig.cap="Length versus weight for brackish species in all_fishdata.", fig.height=1.8, warning=TRUE, echo=FALSE, purl=FALSE}
ggplot(data = brackish_fish, mapping = aes(x = Length, y = Weight)) +
  geom_point()
```

Let's first unpack the graphic in Figure \@ref(fig:noalpha). Observe that a *positive relationship* exists overall between `Length` and `Weight`: as fish lengths increase, fish weights tend to also increase. Observe also the large number of points clustered near (0, 0), which we'll discuss further in Subsection \@ref(overplotting).

Now, let's turn our attention to the warning message. R is alerting us to the fact that many rows were ignored due to missing information. For these rows, either the value for `Length` or `Weight` or both were missing, and thus these rows were ignored in our plot. 

*Missing values* \index{missing values} are encoded as `NA`, which indicates "not available" or "not applicable." If a value for a particular row and a particular column does not exist, `NA` is stored instead. Values can be missing for many reasons. Perhaps the data was collected but someone forgot to enter it. Perhaps the data was not collected at all because it was too difficult to do so. Perhaps there was an erroneous value that someone entered that has been corrected to read as missing. You'll often encounter issues with missing values when working with real data; we'll describe some ways to deal with the missing values in Chapter \@ref(wrangling).
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  What are some practical reasons why `Length` and `Weight` have a positive relationship?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  For which of these two variables, `Length` and `Weight`, is the data more frequently missing?  How did you determine this?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What are some other features of the plot that stand out to you?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create a new scatterplot using different quantitative variables in the `brackish_fish` data frame by modifying the example given.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** After viewing Figure \@ref(fig:alpha), give an approximate range of lengths and weights of fish species that occur most frequently. How has that region changed compared to when you observed the same plot without `alpha = 0.2` set in Figure \@ref(fig:noalpha)?
...
The most common examples of linegraphs have some notion of time on the x-axis: hours, days, weeks, years, etc. Since time is sequential, we connect consecutive observations of the variable on the y-axis with a line. Linegraphs that have some notion of time on the x-axis are also called *time series* plots\index{time series plots}. Let's illustrate linegraphs using a dataset in the `datasets` \index{R packages!datasets} package included in base R: the `ChickWeight` data frame. 

Let's explore the `ChickWeight` data frame by running `View(ChickWeight)` and `glimpse(ChickWeight)`. Furthermore let's read the associated help file by running `?ChickWeight` to bring up the help file. 

Observe that there is a variable called `weight` which records the weight (in grams) for the first 21 days of 50 new born chicks fed one of four different diets (1-4). However, for simplicity let's focus on weights of Chick 1 on Diet 1. 

Recall in Section \@ref(scatterplots), we used the `filter()` function to only choose the subset of rows of `all_fishdata` corresponding to brackish fish. We similarly use `filter()` \index{dplyr!filter()} here, but by using the `&` operator we will choose the subset of rows of `ChickWeight` using two criteria: where the `Chick` is `1` and the `Diet` is `1`. (Since Chick 1 was only fed Diet 1, this is redundant, but it illustrates how to filter on multiple criteria.)
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Take a look at both the `ChickWeight` and `chick1_weight` data frames by running `View(ChickWeight)` and `View(chick1_weight)`. In what respect do these data frames differ?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** `Glimpse()` the `ChickWeight` data frame again. Notice that two of the variables use a data type that you have seen previously. What is the data type? What abbreviations are used for the new data types?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Two of the `ChickWeight` variables use data types that you have not seen previously. What abbreviations are used for the new data types?
...
These two new data types are also used in R for categorical variables, when only a subset of options or "levels" are available. The `<ord>` or "ordered factor" data type is used when the different levels of the categorical variable are in a specifc order, such as Small, Medium, and Large, etc.. The `<fct>` or (unordered) "factor" is used when the levels are not in a specific order, such as Red, Blue, Green, etc.

### Linegraphs via `geom_line` {#geomline}

Let's create a time series plot of the body weights saved in the `chick1_weight` data frame by using `geom_line()` to create a linegraph\index{ggplot2!geom\_line()}, instead of using `geom_point()` like we used previously to create scatterplots:

```{r dailyweight, fig.cap="body weights of Chick 1 on Diet 1"}
ggplot(data = chick1_weight, 
       mapping = aes(x = Time, y = weight)) +
  geom_line()
```

Much as with the `ggplot()` code that created the scatterplot of length and weight for brackish fish in Figure \@ref(fig:noalpha), let's break down this code piece-by-piece in terms of the grammar of graphics:

Within the `ggplot()` function call, we specify two of the components of the grammar of graphics as arguments:

1. The `data` to be the `chick1_weight` data frame by setting `data = chick1_weight`.
1. The `aes`thetic `mapping` by setting `mapping = aes(x = Time, y = weight)`. Specifically, the variable `Time` maps to the `x` position aesthetic, while the variable `weight` maps to the `y` position aesthetic.

We add a layer to the `ggplot()` function call using the `+` sign. The layer in question specifies the third component of the grammar:  the `geom`etric object. In this case, the geometric object is a line set by specifying `geom_line()`. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  Why should linegraphs be avoided when there is not a clear ordering of the x-axis?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why are linegraphs frequently used when time is the explanatory variable on the x-axis?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Plot a time series of weight for a different Chick.
...
Linegraphs, just like scatterplots, display the relationship between two numerical variables. However, it is preferred to use linegraphs over scatterplots when the variable on the x-axis (i.e., the explanatory variable) has an inherent ordering, such as time or altitude.  
...
For example, suppose we were interested in comparing the timeline of weight gain among the 20 chicks fed Diet 1. We could "duplicate" the linegraph above for Chick 1 for the other 19 chicks on Diet 1. In other words, we would plot linegraphs of `weight` for each `Chick` on Diet 1 separately. 

First we create a new data frame by filtering ChickWeight for rows with Chicks on Diet 1:
...
Now we make a linegraph for each chick on Diet 1 by adding `facet_wrap(~ Chick)` layer. Note the `~` is a "tilde" and can generally be found on the key next to the "1" key on US keyboards. The tilde is required and you'll receive an error message if you don't include it here. 
...
```{r facetlinegraph, fig.cap="Faceted histogram of body weights by chicks on Diet 1", echo=FALSE, purl=FALSE, fig.height=3.3}
chick_facet <- ggplot(data = Diet1_ChickWeight, 
       mapping = aes(x = Time, y = weight)) +
  geom_line() +
  facet_wrap(~Chick)
...
We can also specify the number of rows and columns in the grid by using the `nrow` and `ncol` arguments inside of \index{ggplot2!facet\_wrap()} `facet_wrap()`. For example, say we would like our faceted histogram to have 5 rows instead of 4. We simply add an `nrow = 5` argument to `facet_wrap(~ Chick)`.
...
```{r facetlinegraph2, fig.cap="Faceted histogram with 5 instead of 4 rows.", echo=FALSE, purl=FALSE, fig.height=3.3}
chick_facet_5 <- ggplot(data = Diet1_ChickWeight, 
       mapping = aes(x = Time, y = weight)) +
  geom_line() +
  facet_wrap(~Chick, nrow = 5)
...
Observe in both Figures \@ref(fig:facetlinegraph) and \@ref(fig:facetlinegraph2) that as we would expect chick weights increased with time. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What do the numbers above each plot correspond to?  What about the numbers below the bottom plots?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What other things do you notice about this faceted plot?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How does a faceted plot help us see relationships between two variables?
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Does the rate of `weight` gain over time in the `chick_weight_d21` dataset have a lot of variability?  Why do you say that?
...
Let's consider the `weight` variable in the `ChickWeight` data frame once again, but unlike with the linegraphs in Section \@ref(linegraphs), let's say we don't care about its relationship with time, but rather we only care about how the values of `weight` *distribute* in 3-week old chicks. In other words:
...
First, we filter the `ChickWeight` data frame for the desired rows, each chick's weight at Day 21:
...
One way to visualize the *distribution* \index{distribution} of this single variable `weight` is to plot the weights on a horizontal line as we do in Figure \@ref(fig:weight-on-line):

```{r weight-on-line, echo=FALSE, fig.height=0.8, fig.cap="Plot of body weights for Chick 1"}
...
This gives us a general idea of how the values of `weight` distribute: observe that weights vary from around `r round(min(chick_weight_d21$weight, na.rm = TRUE), 0)` grams up to `r round(max(chick_weight_d21$weight, na.rm = TRUE), 0)` grams.  Furthermore, there appear to be more recorded weights between 200 g and 300 g than outside this range. However, because of the high degree of overplotting in the points, it's hard to get a sense of exactly how many values are between say 150 g and 200 g.

What is commonly produced instead of Figure \@ref(fig:weight-on-line) is known as a \index{histograms} *histogram*.  A histogram is a plot that visualizes the *distribution* of a numerical value as follows:
...
ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(binwidth = 20, boundary = 100, color = "white")
```

Let's focus only on weights between 100 g and 200 g for now. Observe that there are five bins of equal width between 100 g and 200 g. Thus we have five bins of width 20 g each: one bin for the 100-120 g range, another bin for the 120-140 g range, etc. Since:

1. The bins for the 100-120 g and 120-140 g ranges each have a height of 1. In other words, one of the Day 21 weight recordings is between 100-120 g and another one is between 120-140 g.
1. The bin for the 140-160 g range has a height of 6. In other words, six of the Day 21 weight recordings are between 140-160 g.

All 16 bins spanning 60 g to 380 g on the x-axis have this interpretation.


### Histograms via `geom_histogram` {#geomhistogram}

Let's now present the `ggplot()` code to plot your first histogram! Unlike with scatterplots and linegraphs, there is now only one variable being mapped in `aes()`: the single numerical variable `weight`. The y-aesthetic of a histogram, the count of the observations in each bin, gets computed for you automatically. Furthermore, the geometric object layer is now a `geom_histogram()`. \index{ggplot2!geom\_histogram()} After running the following code, you'll see the histogram in Figure \@ref(fig:chick-weight-d21-histogram) as well as warning messages. We'll discuss the warning message first. 

```{r chick-weight-d21-histogram, warning=TRUE, fig.cap="Histogram of chick weights at day 21.", fig.height=2.3}
ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram()
```
...
Now let's unpack the resulting histogram in Figure \@ref(fig:chick-weight-d21-histogram). Observe that values less than 150 g as well as values above 325 g are rather rare. However, because of the large number of bins, it's hard to get a sense for which range of weights is spanned by each bin; everything is one giant amorphous blob. So let's add white vertical borders demarcating the bins by adding a `color = "white"` argument to `geom_histogram()` and ignore the warning about setting the number of bins to a better value:

```{r chick-weight-d21-histogram-2, message=FALSE, fig.cap="Histogram of chick weights at day 21.", fig.height=3}
ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(color = "white")
```

We now have an easier time associating ranges of weights to each of the bins in Figure \@ref(fig:chick-weight-d21-histogram-2). We can also vary the color of the bars by setting the \index{ggplot2!fill} `fill` argument. For example, you can set the bin colors to be "blue steel" by setting `fill = "steelblue"`:
...
Observe in Figure \@ref(fig:chick-weight-d21-histogram-2) that in the 100-200 g range there appear to be about 10 bins. Thus each bin has width 100 divided by about 10, or  about 10 g. Let's improve this histogram by adjusting the number of bins in our histogram in one of two ways:
...
Using the second method, instead of specifying the number of bins, we specify the width of the bins by using the `binwidth` argument in the `geom_histogram()` layer. For example, let's set the width of each bin to be 20 g.
...
hist_1 <- ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(bins = 20, color = "white") +
  labs(title = "With 20 bins")
hist_2 <- ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(binwidth = 10, color = "white") +
  labs(title = "With binwidth = 20 g")
hist_1 + hist_2
```
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What does changing the number of bins from 30 to 20 tell us about the distribution of weights?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Would you classify the distribution of weights as symmetric or skewed in one direction or another?
...
```{r, echo=FALSE, purl=FALSE}
n_Diet1 <- ChickWeight %>%
  filter(Diet == 1 & Time == 21) %>%
  nrow()
min_Diet1 <- ChickWeight %>%
  filter(Diet == 1 & Time == 21) %>%
  pull(weight) %>%
  min(na.rm = TRUE) %>%
  round(0)
max_Diet1 <- ChickWeight %>%
  filter(Diet == 1 & Time == 21) %>%
  pull(weight) %>%
  max(na.rm = TRUE) %>%
  round(0)
quartiles <- ChickWeight %>%
  filter(Diet == 1 & Time == 21) %>%
  pull(weight) %>%
  quantile(prob = c(0.25, 0.5, 0.75)) %>%
  round(0)
five_number_summary <- tibble(
  weight = c(min_Diet1, quartiles, max_Diet1)
)
```

While faceted histograms are one type of visualization used to compare the distribution of a numerical variable split by the values of another variable, another type of visualization that achieves this same goal is a *side-by-side boxplot*. A boxplot \index{boxplots} is constructed from the information provided in the *five-number summary* of a numerical variable: minimum, first quartile, median, third quartile, and maximum values (see Appendix \@ref(appendix-stat-terms)).  

To keep things simple for now, let's only consider the Day 21 weight recordings for the `r n_Diet1` chicks on Diet 1, each represented as a jittered point in Figure \@ref(fig:Diet11). 

```{r Diet11, echo=FALSE, fig.cap="Day 21 weights of Diet 1 chicks represented as jittered points.", fig.height=1.7, purl=FALSE}
base_plot <- ChickWeight %>%
  filter(Diet == 1 & Time == 21) %>%
  ggplot(mapping = aes(x = factor(Diet), y = weight)) +
  labs(x = "")
base_plot +
  geom_jitter(width = 0.075, height = 0.5, alpha = 0.3)
```

These `r n_Diet1` observations have the following *five-number summary*:

1. Minimum: `r min_Diet1` g
1. First quartile (25th percentile): `r quartiles[1]` g
1. Median (second quartile, 50th percentile): `r quartiles[2]` g
1. Third quartile (75th percentile): `r quartiles[3]` g
1. Maximum: `r max_Diet1` g

In the leftmost plot of Figure \@ref(fig:Diet12), let's mark these 5 values with dashed horizontal lines on top of the `r n_Diet1` points. In the middle plot of Figure \@ref(fig:Diet12) let's add the *boxplot*. In the rightmost plot of Figure \@ref(fig:Diet12), let's remove the points and the dashed horizontal lines for clarity's sake.

```{r Diet12, echo=FALSE, fig.cap="Building up a boxplot of Diet 1 chick weights.", fig.height=3, purl=FALSE}
boxplot_1 <- base_plot +
  geom_hline(data = five_number_summary, aes(yintercept = weight), linetype = "dashed") +
...
What the boxplot does is visually summarize the `r ChickWeight %>% filter(Diet == 1 & Time == 21) %>% nrow()` points by cutting the `r n_Diet1` weight recordings into *quartiles* at the dashed lines, where each quartile contains roughly `r n_Diet1` $\div$ 4 $\approx$ `r round(n_Diet1 / 4)` observations. Thus
...
1. 25% of points fall below the bottom edge of the box, which is the first quartile of `r quartile_1` g. In other words, 25% of observations were below `r quartile_1` g.
1. 25% of points fall between the bottom edge of the box and the solid middle line, which is the median of `r quartile_2` g. Thus, 25% of observations were between `r quartile_1` g and `r quartile_2` g and 50% of observations were below `r quartile_2` g.
1. 25% of points fall between the solid middle line and the top edge of the box, which is the third quartile of `r quartile_3` g. It follows that 25% of observations were between `r quartile_2` g and `r quartile_3` g and 75% of observations were below `r quartile_3` g.
1. 25% of points fall above the top edge of the box. In other words, 25% of observations were above `r quartile_3` g.
1. The middle 50% of points lie within the *interquartile range (IQR)* \index{interquartile range (IQR)} between the first and third quartile. Thus, the IQR for this example is `r quartile_3` - `r quartile_1` = `r (quartiles[3] - quartiles[1]) %>% round(3)` g. The interquartile range is a measure of a numerical variable's *spread*.

Furthermore, in the rightmost plot of Figure \@ref(fig:Diet12), we see the *whiskers* \index{boxplots!whiskers} of the boxplot. The whiskers stick out from either end of the box all the way to the minimum and maximum observed weights of `r min_Diet1` g and `r max_Diet1` g, respectively. However, the whiskers don't always extend to the smallest and largest observed values as they do here. They in fact extend no more than 1.5 $\times$ the interquartile range from either end of the box. In this case of the Diet 1 weights, no more than 1.5 $\times$ `r (quartiles[3] - quartiles[1]) %>% round(3)` g = `r (1.5*(quartiles[3] - quartiles[1])) %>% round(3)` g from either end of the box. Any observed values outside this range get marked with points called *outliers*, which we'll see in the next section.


### Boxplots via `geom_boxplot` {#geomboxplot}

Let's now create a side-by-side boxplot \index{boxplots!side-by-side} of Day 21 weights split by the four diets. We do this by first filtering for the rows where `Time` equals `21` days. We then plot these values, mapping the `Diet` variable to the x-position aesthetic, the `weight` variable to the y-position aesthetic, and adding a `geom_boxplot()` layer:

```{r Dietweightbox, fig.cap="Side-by-side boxplot of weight split by Diet.", fig.height=2.4}
...
Note that boxplots require a categorical -- not a numerical -- variable to be mapped to the x-position aesthetic. Although the Diets are numbered, recall that this variable is stored as a factor (`<fct>`) data type, which you can confirm using `glimpse(ChickWeight)`. So R knows that this is a categorical variable on the x-position aesthetic. If the variable `Diet` had been stored as a `<num>` data type instead, we would have needed to convert it into a categorical variable by using the `factor()` \index{factors} function.

The resulting Figure \@ref(fig:Dietweightbox) shows four separate "box and whiskers" plots similar to the rightmost plot of Figure \@ref(fig:Diet12) of only Diet 1 weights. Thus the different boxplots are shown "side-by-side."
...
If any values had been more than $1.5 \times IQR$ away from either box end and therefore beyond the whiskers, then they would have been indicated by dots. These so-called \index{outliers} *outliers* can be thought of as anomalous ("out-of-the-ordinary") values. It is important to keep in mind that the definition of an outlier is somewhat arbitrary and not absolute. In these boxplots, they are defined by the length of the whiskers, which are no more than $1.5 \times IQR$ units long for each boxplot. 
<!--Perhaps replace dataset to include outliers-->

Looking at this side-by-side plot we can see that Diet 3 produced chicks with the highest median weight at Day 21 as evidenced by the higher solid line in the middle of its box. We can easily compare weights across diets by drawing imaginary horizontal lines across the plot. Furthermore, the heights of the four boxes as quantified by the interquartile ranges are informative too; they tell us about variability, or spread, of weights recorded for a given diet. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Which diet produced the greatest variability in weights?  Explain your reasoning.
...
Let's now go back to the `all_fishdata` data frame from earlier (Subsection \@ref(fishbasedataframe)) and visualize the distribution of the categorical variable `DemersPelag`, which indicates the preferred habitat for each fish species. In other words, let's visualize the number of fish species preferring each of the following environments: benthopelagic, pelagic,  demersal, pelagic-neritic, reef-associated,  bathypelagic, bathydemersal, and pelagic-oceanic (see the [fishbase manual](https://www.fishbase.se/manual/english/fishbasethe_species_table.htm) for a description of each habitat).   Recall from Subsection \@ref(exploredataframes) when you first explored the `all_fishdata` data frame, you saw that each row corresponds to a fish species. In other words, the `all_fishdata` data frame is more like the `fruits` data frame than the `fruits_counted` data frame because the fish species have not been pre-counted by `DemersPelag`. Thus we should use `geom_bar()` instead of `geom_col()` to create a barplot. Much like a `geom_histogram()`, there is only one variable in the `aes()` aesthetic mapping: the variable `DemersPelag` gets mapped to the `x`-position. Note that the last line of R code rotates and shifts the labels so that they don't overlap.

(ref:geombar) Number of fish species preferring each habitat using geom_bar().

```{r habitatsbar, fig.cap="(ref:geombar)", fig.height=2.8}
ggplot(data = all_fishdata, mapping = aes(x = DemersPelag)) +
  geom_bar() + 
  theme(axis.text.x=element_text(angle=30,hjust=1))
```

Observe in Figure \@ref(fig:habitatsbar) that demersal, benthopelagic, and reef-associated environments were the preferred habitat for the most fish species. Also notice that bar graphs have white space between the bars whereas histograms have bars that touch.

Alternatively, you may have a data frame where the number of fish species preferring each habitat (the `DemersPelag` variable) was pre-counted as in Table \@ref(tab:all-fishdata-counted).

```{r all-fishdata-counted, message=FALSE, echo=FALSE, purl=FALSE}
fish_counted <- all_fishdata %>%
  group_by(DemersPelag) %>%
  summarize(number = n())
kable(fish_counted,
  digits = 3,
  caption = "Number of fish species pre-counted by preferred habitat.",
  booktabs = TRUE,
  longtable = TRUE,
  linesep = ""
) %>%
...
In order to create a barplot visualizing the distribution of the categorical variable `DemersPelag` in this case, we would now use `geom_col()` instead of `geom_bar()`, with an additional `y = number` in the aesthetic mapping on top of the `x = DemersPelag`. The resulting barplot would be identical to Figure \@ref(fig:habitatsbar).
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How many fish species prefer bathypelagic environments?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What was the 4th most preferred habitat for fish species? How could we rearrange the table to get this answer more quickly?
...
Barplots are a very common way to visualize the frequency of different categories, or levels, of a single categorical variable. Another use of barplots is to visualize the *joint* distribution of two categorical variables at the same time.  Let's examine the *joint* distribution of fish species by `DemersPelag` as well as `BodyShapeI`. In other words, the number of fish species for each `DemersPelag` and `BodyShapeI` combination. 

For example, the number of fish species preferring a bathydemersal habitat with a fusiform/normal shape, the number preferring  a bathydemersal habitat with an elongated shape, and so on. Recall the `ggplot()` code that created the barplot of `DemersPelag` frequency in Figure \@ref(fig:habitatsbar):
...
We can now map the additional variable `BodyShapeI` by adding `fill = BodyShape1` inside the `aes()` aesthetic mapping.
...
```{r all-fishdata-stacked-bar, echo=FALSE, fig.cap="Stacked barplot of fish species by preferred habitat (DemersPelag) and body shape (BodyShapeI).", fig.height=2.8, purl=FALSE}
if (is_html_output()) {
  ggplot(data = all_fishdata, mapping = aes(x = DemersPelag, fill = BodyShapeI)) +
    geom_bar() + 
  theme(axis.text.x=element_text(angle=30,hjust=1))
...
Figure \@ref(fig:all-fishdata-stacked-bar) is an example of a \index{barplot!stacked} *stacked barplot*.  While simple to make, in certain aspects it is not ideal. For example, it is difficult to compare the heights of the different colors between the bars, corresponding to comparing the number of fish species with each `BodyShapeI` type between the preferred `DemersPelag` habitats.

Before we continue, let's address some common points of confusion among new R users. First, the `fill` aesthetic corresponds to the color used to fill the bars, while the `color` aesthetic corresponds to the color of the outline of the bars. This is identical to how we added color to our histogram in Subsection \@ref(geomhistogram): we set the outline of the bars to white by setting `color = "white"` and the colors of the bars to blue steel by setting `fill = "steelblue"`. Observe in Figure \@ref(fig:all-fishdata-stacked-bar-color) that mapping `BodyShapeI` to `color` and not `fill` yields grey bars with different colored outlines.
...
```{r all-fishdata-stacked-bar-color, fig.cap="Stacked barplot with color aesthetic used instead of fill.", fig.height=2.2, echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = all_fishdata, mapping = aes(x = DemersPelag, color = BodyShapeI)) +
    geom_bar() + 
  theme(axis.text.x=element_text(angle=30,hjust=1))
...
An alternative to stacked barplots are \index{barplot!side-by-side} *side-by-side barplots*, also known as *dodged barplots*, as seen in Figure \@ref(fig:all-fishdata-dodged-bar-color). The code to create a side-by-side barplot is identical to the code to create a stacked barplot, but with a \index{ggplot2!position} `position = "dodge"` argument added to `geom_bar()`. In other words, we are overriding the default barplot type, which is a *stacked* barplot, and specifying it to be a side-by-side barplot instead.
...
```{r all-fishdata-dodged-bar-color, fig.cap="Side-by-side barplot comparing number of fish species by DemersPelag and BodyShapeI variables.", fig.height=3.5, echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = all_fishdata, mapping = aes(x = DemersPelag, fill = BodyShapeI)) +
    geom_bar(position = "dodge") + 
  theme(axis.text.x=element_text(angle=30,hjust=1))
...
```
<!--
Note the width of the bars differs; for example, the bars for `pelagic` are wider than for `demersal`. We can make one tweak to the `position` argument to get them to be the same size in terms of width as the other bars by using the more robust `position_dodge()` function.
...
```{r all-fishdata-dodged-bar-color-tweak, echo=FALSE, fig.cap="Side-by-side barplot comparing number of all_fishdata by DemersPelag and BodyShapeI (with formatting tweak).", fig.height=2.5, purl=FALSE}
if (is_html_output()) {
  ggplot(data = all_fishdata, mapping = aes(x = DemersPelag, fill = BodyShapeI)) +
    geom_bar(position = position_dodge(preserve = "single"))
} else {
...
Another another type of graph to visualize two categorical variables at the same time is a \index{barplot!faceted} *faceted barplot*. Recall in Section \@ref(facets) we visualized the distribution of weight gain for the chicks on Diet 1 *split* by the Chick identifier (1-50) using facets. We apply the same principle to our barplot visualizing the frequency of `DemersPelag` split by `BodyShapeI`: instead of mapping `BodyShapeI` to `fill` we include it as the variable to create small multiples of the plot across the levels of `BodyShapeI`.
...
```{r facet-bar-vert, fig.cap="Faceted barplot comparing the number of fish species by the DemersPelag and BodyShapeI variables.", fig.height=6, echo=FALSE, purl=FALSE}
shape_facet_ncol <- ggplot(data = all_fishdata, mapping = aes(x = DemersPelag)) +
  geom_bar() +
  facet_wrap(~ BodyShapeI, ncol = 1) + 
  theme(axis.text.x=element_text(angle=30,hjust=1))
...
Finally, in Section \@ref(ht-activity), we will visualize the joint frequency of two categorical variables at the same time using a *mosaic* plot.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What kinds of questions are not easily answered by looking at Figure \@ref(fig:all-fishdata-stacked-bar)?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What can you say, if anything, about the relationship between preferred habitat and body shape in regards to the number of fish species?
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What information about the different preferred habitats of different body shapes is more easily seen in the faceted barplot?
...
When we visualize data, the goal is to display the key features as clearly and succinctly as possible. These key features may include the central location of our data (e.g., median), its spread (e.g., IQR), and the sample size of each group. Depending on the data to display (numerical/categorical, one/multiple variables), some plots are more effective at presenting the key features of the data. In this section, we describe two plots that are frequently seen in the scientific literature but are less effective than common alternatives and should therefore be avoided. 

### Barplots with a continuous variable

The first plot to avoid is a bar plot to display the distribution of a *continuous* (numerical) variable. But wait, then why was Section \@ref(geombar) all about how to create bar plots? Recall from the opening paragraph of Section \@ref(geombar) that barplots should be used to visualize the distribution of *categorical* variables, while histograms and boxplots should be used to visualize the distribution of *numerical* variables. Despite this, a recent study [@Riedel2022] found that bar plots are more often used to inappropriately display continuous data than they are used appropriately to display count data. 

So, what is the problem with using barplots to display continuous data? As shown in Figure \@ref(fig:barplot-anatomy), bar graphs hide important information that can be easily indicated using a scatterplot (dotplot) or boxplot. While the bar graph displays the mean of the data and some indication of its spread, it hides much of the data (zone of invisibility). In contrast, the dotplot succinctly presents the *complete* distribution of the numerical variable *and* indicates the sample size of each group. Furthermore, the bar graph includes a lot of "dead space" (zone of irrelevance) distracting our attention from the difference between means compared to the spread. For these reasons, more and more scientific journals are banning the use of barplots to display continuous data -- and you should too. 

(ref:WEISSGERBER2017) Bar graphs hide important information about continuous data. Source: [@WEISSGERBER201720592], CC BY 4.0.

```{r barplot-anatomy, echo=FALSE, fig.cap="(ref:WEISSGERBER2017)", fig.height=6, echo=FALSE, purl=FALSE}
include_graphics("images/copyright/gr4_lrg.png")
```   
...
Let's examine the same data used in our previous barplot of the number of fish species preferring each `DemersPelag` environment in Figure \@ref(fig:habitatsbar), but this time we will use a pie chart in Figure \@ref(fig:DemersPelagpie). Try to answer the following questions:

* How much larger is the portion of the pie for bathydemersal compared to bathypelagic? 
* What is the fifth most preferred `DemersPelag` habitat for fish species?

```{r DemersPelagpie, echo=FALSE, fig.cap="The dreaded pie chart.", fig.height=4.8, purl=FALSE}
if (is_html_output()) {
  ggplot(all_fishdata, mapping = aes(x = factor(1), fill = DemersPelag)) +
    geom_bar(width = 1) +
    coord_polar(theta = "y") +
    theme(
...
While it is quite difficult to answer these questions when looking at the pie chart in Figure \@ref(fig:DemersPelagpie), we can much more easily answer these questions using the barchart in Figure \@ref(fig:habitatsbar). This is true since barplots present the information in a way such that comparisons between categories can be made with single horizontal lines, whereas pie charts present the information in a way such that comparisons must be made by \index{pie charts!problems with} comparing angles. 
...
If you want to further unlock the power of the `ggplot2` package for data visualization, we suggest that you check out RStudio's "Data Visualization with ggplot2" cheatsheet. This cheatsheet summarizes much more than what we've discussed in this chapter. (For example in Figure \@ref(fig:habitatsbar), you saw that we could add code to rotate labels.) In particular, the ggplot2 cheatsheet presents many more than the 5 `geom`etric objects we covered in this chapter while providing quick and easy to read visual descriptions. For all the `geom`etric objects, it also lists all the possible aesthetic attributes one can tweak. In the current version of RStudio, you can access this cheatsheet by going to the RStudio Menu Bar -> Help -> Cheat Sheets -> "Data Visualization with ggplot2." `r if(is_html_output()) "You can see a preview in the figure below."`
...
Recall in Figure \@ref(fig:noalpha) in Section \@ref(scatterplots) we visualized the relationship between length and weight of brackish fish species in all_fishdata. This necessitated paring down the `all_fishdata` data frame to a new data frame `brackish_fish` consisting of only `Brack == 1` fish species first:
...
Furthermore recall in Figure \@ref(fig:dailyweight) in Section \@ref(linegraphs) we visualized weight gain over time for Chick 1 on Diet 1.  This necessitated paring down the `ChickWeight` data frame to a new data frame `chick1_weight` consisting of body weight recordings only for `Chick == 1`, and `Diet == 1`:
...
These two code segments were a preview of Chapter \@ref(wrangling) on data wrangling using the `dplyr` package. Data wrangling is the process of transforming and modifying existing data with the intent of making it more appropriate for analysis purposes. For example, these two code segments used the `filter()` function to create new data frames (`brackish_fish` and `chick1_weight`) by choosing only a subset of rows of existing data frames (`all_fishdata` and `ChickWeight`). In the next chapter, we'll formally introduce the `filter()` and other data wrangling functions as well as the *pipe operator* `%>%` which allows you to combine multiple data wrangling actions into a single sequential *chain* of actions. On to Chapter \@ref(wrangling) on data wrangling!
