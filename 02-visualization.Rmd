...
1. `brain_wt`: brain weight (in kg) on the horizontal "x" axis and
1. `life_span`: total number of hours of sleep on the vertical "y" axis  
for each mammalian species.


```{r purl=FALSE, echo=FALSE}
# This redundant code is used for dynamic non-static in-line text output purposes
species_rows <- mammals %>%
  nrow() %>%
  comma()
brack_species_rows <- mammals %>%
  nrow() %>%
  comma()
```
...
1. The `data` as the `mammals` data frame via `data = mammals`.
1. The `aes`thetic \index{ggplot2!mapping} `mapping` by setting `mapping = aes(x = brain_wt, y = life_span)`. Specifically, the variable `brain_wt` maps to the `x` position aesthetic, while the variable `life_span` maps to the `y` position.
...
```{r noalpha, fig.cap="Brain weight vs. life span in mammals data frame.", fig.height=1.8, warning=TRUE, echo=FALSE, purl=FALSE}
ggplot(data = mammals, mapping = aes(x = brain_wt, y = life_span)) + 
  geom_point()
```

Let's first unpack the graphic in Figure \@ref(fig:noalpha). Observe that a *positive relationship* exists overall between `brain_wt` and `life_span`: as `brain_wt` increases, the `life_span` tends to also increase. Observe also the large number of points clustered near (0, 0), which we'll discuss further in Subsection \@ref(overplotting).

Now, let's turn our attention to the warning message. R is alerting us to the fact that some rows were ignored due to missing information. For these rows, either the value for `brain_wt` or `life_span` or both were missing, and thus these rows were ignored in our plot. 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  What are some practical reasons why `brain_wt` and `life_span` have a positive relationship?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`**  For which of these two variables, `brain_wt` and `life_span`, is the data more frequently missing?  How did you determine this?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What are some other features of the plot that stand out to you?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Create a new scatterplot using different quantitative variables in the `mammals` data frame by modifying the example given.
...
```{r alpha, fig.cap="Brain weight vs. life span scatterplot with alpha = 0.2.", fig.height=4.9}
ggplot(data = mammals, mapping = aes(x = brain_wt, y = life_span)) + 
...
```{r jitter, fig.cap="Brain weight vs. life span jittered scatterplot.", fig.height=4.7}
ggplot(data = mammals, mapping = aes(x = brain_wt, y = life_span)) + 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** After viewing Figure \@ref(fig:alpha), give an approximate range of brain weights and life spans that occur most frequently in mammalian species. How has that region changed compared to when you observed the same plot without `alpha = 0.2` set in Figure \@ref(fig:noalpha)?
...
Observe that there is a variable called `weight` which records the weight (in grams) for the first 21 days of 50 new born chicks fed one of four different diets (1-4). However, for simplicity let's focus on weights of Chick 1. 

To focus on Chick 1, we will use `dplyr's` `filter()` \index{dplyr!filter()} function to choose the subset of rows in `ChickWeight` corresponding to Chick 1. Recall from Section \@ref(code) that testing for equality is specified with `==` and not `=`. If you don't fully understand the code, don't worry for now; we'll cover the `filter` function in more depth in Chapter \@ref(wrangling) on data wrangling.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** `Glimpse()` the `ChickWeight` data frame again. Notice that two of the variables use a data type that you have seen previously. What are the data types? 

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** One of the `ChickWeight` variables uses a data type that you have not seen previously. What abbreviation is used for the new data type?
...
These `ord` and `fct` data types are used in R for categorical variables, when only a subset of options or "levels" are available. The `<ord>` or "ordered factor" data type is used when the different levels of the categorical variable are in a specific order, such as Small, Medium, and Large, etc.. The `<fct>` or (unordered) "factor" is used when the levels are not in a specific order, such as Red, Blue, Green, etc.
...
Much as with the `ggplot()` code that created the scatterplot of brain weight and life span for mammals in Figure \@ref(fig:noalpha), let's break down this code piece-by-piece in terms of the grammar of graphics:
...
This gives us a general idea of how the values of `weight` distribute: observe that weights vary from around `r round(min(chick_weight_d21$weight, na.rm = TRUE), -1)` grams up to `r round(max(chick_weight_d21$weight, na.rm = TRUE), -1)` grams.  Furthermore, there appear to be more recorded weights between 150 g and 250 g than outside this range. However, because of the high degree of overplotting in the points, it's hard to get a sense of exactly how many values are between say 150 g and 200 g.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Is this data spread out greatly from the center or is it close?  Explain your answer.
...
For example, suppose we were interested in comparing the Day 21 weights for chicks on the four different diets. We could "duplicate" the histogram above but plot histograms of `weight` on Day 21 for each `Diet` separately. We make a histogram for chick weights on each diet by adding a `facet_wrap(~ Diet)` layer. Note the `~` is a "tilde" and can generally be found on the key next to the "1" key on US keyboards. The tilde is required and you'll receive an error message if you don't include it here. 
...
chick_facet <- ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(bins = 20, color = "white") +
  facet_wrap(~Diet)
...
We can also specify the number of rows and columns in the grid by using the `nrow` and `ncol` arguments inside of \index{ggplot2!facet\_wrap()} `facet_wrap()`. For example, say we would like our faceted histogram to have one column for easier comparisons. We simply add an `ncol = 1` argument to `facet_wrap(~ Chick)`.
...
```{r facetlinegraph2, fig.cap="Faceted histogram with 1 instead of 2 columns.", echo=FALSE, purl=FALSE, fig.height=3.3}
chick_facet_5 <- ggplot(data = chick_weight_d21, mapping = aes(x = weight)) +
  geom_histogram(bins = 20, color = "white") +
  facet_wrap(~Diet, ncol = 1)
...
While we can compare the distribution of chick weights on the four different diets in both Figure \@ref(fig:facetlinegraph) and Figure \@ref(fig:facetlinegraph2), the comparison is easier in the second faceted graph . 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Does the `weight` gain on each diet in the `chick_weight_d21` dataset have a lot of variability?  Why do you say that?
...
Histograms, unlike scatterplots and linegraphs, present information on only a single numerical variable. Specifically, they are visualizations of the distribution of the numerical variable in question. Faceting splits a particular visualization by the values of another variable.
...
While faceted histograms are one type of visualization used to compare the distribution of a numerical variable split by the values of another variable, another type of visualization that achieves this same goal is a *side-by-side boxplot*. A boxplot \index{boxplots} is constructed based on the *five-number summary* of the distribution of a numerical variable: that is, the minimum, first quartile (25th percentile), median (50th percentile), third quartile (75th percentile), and maximum values (described in Appendix \@ref(appendix-stat-terms)).  
...
1. First quartile: `r quartiles[1]` g
1. Median: `r quartiles[2]` g
1. Third quartile: `r quartiles[3]` g
...
Let's now go back to the `mammals` data frame from earlier (Subsection \@ref(mammalsdb)) and visualize the distribution of the categorical variable `predation`, which indicates the likelihood of the mammalian species to be preyed upon (1, lowest; 5, highest). In other words, let's visualize the number of mammalian species in each of the 5 `predation` categories.   Recall from Subsection \@ref(exploredataframes) when you first explored the `mammals` data frame, you saw that each row corresponds to a species. In other words, the `mammals` data frame is more like the `fruits` data frame than the `fruits_counted` data frame because the species have not been pre-counted by `predation`. Thus we should use `geom_bar()` instead of `geom_col()` to create a barplot. Much like a `geom_histogram()`, there is only one variable in the `aes()` aesthetic mapping: the variable `predation` gets mapped to the `x`-position. 

(ref:geombar) Number of species in each predation category using geom_bar().
...
Observe in Figure \@ref(fig:habitatsbar) that `predation` category 4 has the fewest species. Also notice that bar graphs have white space between the bars whereas histograms have bars that touch.

Alternatively, you may have a data frame where the number of mammalian species in each `predation` category was pre-counted as in Table \@ref(tab:all-fishdata-counted).

```{r all-fishdata-counted, message=FALSE, echo=FALSE, purl=FALSE}
mammals_counted <- mammals %>%
  group_by(predation) %>%
  summarize(number = n())
kable(mammals_counted,
  digits = 3,
  caption = "Number of mammalian species pre-counted by predation category.",
  booktabs = TRUE,
  longtable = TRUE,
  linesep = ""
) %>%
...
In order to create a barplot visualizing the distribution of the categorical variable `predation` in this case, we would now use `geom_col()` instead of `geom_bar()`, with an additional `y = number` in the aesthetic mapping on top of the `x = predation`. The resulting barplot would be identical to Figure \@ref(fig:habitatsbar).
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** How are histograms and barplots similar? How do they differ?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What was the 2nd most common `predation` category for the mammalian species? How could we rearrange the table to get this answer more quickly?
...
We can now map the additional variable `exposure` by adding `fill = factor(exposure)` inside the `aes()` aesthetic mapping. Note that we use the `factor()` function here because the `fill` argument is expecting a factor, so we *coerced*  `exposure` from `int` to the `fct` data type.
...
```{r all-fishdata-stacked-bar, echo=FALSE, fig.cap="Stacked barplot of mammalian species by predation category and exposure index.", fig.height=2.8, purl=FALSE}
if (is_html_output()) {
  ggplot(data = mammals, mapping = aes(x = predation, fill = factor(exposure))) +
    geom_bar() 
} else {
...
Figure \@ref(fig:all-fishdata-stacked-bar) is an example of a \index{barplot!stacked} *stacked barplot*.  While simple to make, in certain aspects it is not ideal. For example, it is difficult to compare the heights of the different colors between the bars, corresponding to comparing the number of species with each `exposure` type between the `predation` categories.

Before we continue, let's address some common points of confusion among new R users. First, the `fill` aesthetic corresponds to the color used to fill the bars, while the `color` aesthetic corresponds to the color of the outline of the bars. This is identical to how we added color to our histogram in Subsection \@ref(geomhistogram): we set the outline of the bars to white by setting `color = "white"` and the colors of the bars to blue steel by setting `fill = "steelblue"`. Observe in Figure \@ref(fig:all-fishdata-stacked-bar-color) that mapping `exposure` to `color` and not `fill` yields grey bars with different colored outlines.
...
```{r all-fishdata-dodged-bar-color, fig.cap="Side-by-side barplot comparing number of species by predation and exposure variables.", fig.height=3.5, echo=FALSE, purl=FALSE}
if (is_html_output()) {
  ggplot(data = mammals, mapping = aes(x = predation, fill = factor(exposure))) +
    geom_bar(position = "dodge") 
} else {
...
Another type of graph to visualize two categorical variables at the same time is a \index{barplot!faceted} *faceted barplot*. Recall in Section \@ref(facets) we visualized the distribution of weight gain for the chicks on Diet 1 *split* by the Chick identifier (1-50) using facets. We apply the same principle to our barplot visualizing the frequency of `predation` split by `exposure`: instead of mapping `exposure` to `fill`, we include it as the variable to create small multiples of the plot across the levels of `exposure`.
...
```{r facet-bar-vert, fig.cap="Faceted barplot comparing the number of species by the predation and exposure variables.", fig.height=6, echo=FALSE, purl=FALSE}
shape_facet_ncol <- ggplot(data = mammals, mapping = aes(x = predation)) +
  geom_bar() +
  facet_wrap(~ exposure, ncol = 1) 
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What can you say, if anything, about the relationship between `predation` category and `exposure` index in regards to the number of mammalian species?
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** What information about the different exposure indexes of different  predation categories is more easily seen in the faceted barplot?
...
```{block lc-barplots, type="learncheck", purl=FALSE}
\vspace{-0.15in}
**_Learning check_**
\vspace{-0.1in}
```

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why should barplots be avoided when displaying continuous data?

**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Why do you think people continue to use barplots when displaying continuous data
...
Let's examine the same data used in our previous barplot of the number of mammalian species preferring each `predation` environment in Figure \@ref(fig:habitatsbar), but this time we will use a pie chart in Figure \@ref(fig:predationpie). Try to answer the following questions:

* How much larger is the portion of the pie for predation category 2 compared to category 4? 
* What is the second most common `predation` category for mammalian species?

```{r predationpie, echo=FALSE, fig.cap="The dreaded pie chart.", fig.height=4.8, purl=FALSE}
if (is_html_output()) {
  ggplot(mammals, mapping = aes(x = factor(1), fill = factor(predation))) +
    geom_bar(width = 1) +
    coord_polar(theta = "y") +
    theme(
...
if (!file.exists("rds/summary_table_ch3.rds")) {
  ...
  summary_table_ch3$Notes[3] <- "Like boxplots, facetted histograms show distribution of 1 numerical variable split by the values of a categorical variable"
  summary_table_ch3$Shows[4] <- "Distribution of 1 numerical variable split by the values of a categorical variable"  
