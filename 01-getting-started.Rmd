...
### `fishbase` data frame {#fishbasedataframe}
...
In addition to datasets built into packages, we can also import data from external sources using functions. For example, the `species()` function of the `rfishbase` \index{R packages!rfishbase} package is used to access an online data table at FishBase.org. Run the following two lines of code by typing or copying-and-pasting in your console.
...
Typing the name of the data frame `all_fishdata` in the second line of code, then displays the data. Unfortunately, this output does not allow us to explore the data very well, but it does give a nice preview. Let's look at some different ways to explore data frames more closely.
...
Another nice feature of R are help files, which provide documentation for various functions and datasets. You can bring up help files by adding a `?` \index{operators!?} before the name of a function or data frame and then run this in the console. You will then be presented with a page showing the corresponding documentation if it exists.  For example, let's look at the help file for the `fishbase` data frame and the `species` function.
