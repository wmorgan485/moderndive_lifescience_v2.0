...
Let's take a look at only the `DepthRangeShallow`, `DepthRangeDeep`, and the resulting `depth_range` variables of our updated `all_fishdata` data frame, after removing rows with missing data (indicated by "NA"). The first 5 rows are shown in Table \@ref(tab:first-five-all-fishdata).
...
# values for the text
first_shallow = all_fishdata %>% 
  select(DepthRangeShallow, DepthRangeDeep, depth_range) %>% 
    drop_na() %>% slice(1) %>% pull(DepthRangeShallow)
first_deep = all_fishdata %>% 
  select(DepthRangeShallow, DepthRangeDeep, depth_range) %>% 
    drop_na() %>% slice(1) %>% pull(DepthRangeDeep)
```

The deepest depth of the fish species in the first row is `r first_deep` meters and its shallowest is `r first_shallow`, so the difference between the two depths, its `depth_range` is `r first_deep` - `r first_shallow` = `r first_deep - first_shallow` meters. 
