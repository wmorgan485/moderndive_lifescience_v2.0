...
BirdMalaria_barplot <- ggplot(BirdMalaria, aes(x = treatment, fill = response)) +
  geom_bar() +
  labs(x = "Treatment")
if (is_html_output()) {
  BirdMalaria_barplot
} else {
  BirdMalaria_barplot + scale_fill_grey()
}
```
...
BirdMalaria_mosaicplot <- ggplot(BirdMalaria) +
  geom_mosaic(aes(x = product(treatment), 
                  fill = response)) +
  labs(x = "Treatment", y = "Relative frequency")
if (is_html_output()) {
  BirdMalaria_mosaicplot
} else {
  BirdMalaria_mosaicplot + scale_fill_grey()
}
```
...
```{r, echo=FALSE, purl=FALSE}
observed_test_statistic <- BirdMalaria %>%
...
To run the simulations, we will work with the original `BirdMalaria` dataset. To understand the probability of the observed results, first try to imagine a hypothetical universe where egg removal didn't affect the likelihood of a malaria infection. In such a hypothetical universe, the nest status of a female bird would have no bearing on their chances of becoming infected by a parasite. Bringing things back to our `BirdMalaria` data frame, the `treatment` variable would thus be an irrelevant label for predicting malaria infection. If these `treatment` labels were irrelevant, then we could randomly reassign them by "shuffling" them to no consequence!

To illustrate this idea, let's narrow our focus to 6 arbitrarily chosen females of the `r total_birds` in Table \@ref(tab:compare-six). The malaria column shows that 3 females were infected while 3 weren't. The `treatment` column shows the nest treatment, control or egg removal. 
...
set.seed(16)
# Pick out 6 rows
BirdMalaria_sample <- BirdMalaria %>%
...
We've saved one such shuffling in the `BirdMalaria_shuffled` data frame. If you compare the original `BirdMalaria` and the shuffled `BirdMalaria_shuffled` data frames, you'll see that while the malaria variable is identical, the `shuffled_treatment` variable differs from `treatment`. 

Let's repeat the same exploratory data analysis we did for the original `BirdMalaria` data on our `BirdMalaria_shuffled` data frame. Let's create a bar plot visualizing the relationship between malaria and the new shuffled `treatment` variable and compare this to the original unshuffled version in Figure \@ref(fig:BirdMalaria-barplot-permuted).
...
BirdMalaria_shuffled <- BirdMalaria %>%
  mutate(`shuffled_treatment` = sample(treatment))
height1 <- BirdMalaria %>%
  group_by(treatment) %>%
  summarize(n = n()) %>%
...
plot1 <- ggplot(BirdMalaria, aes(x = treatment, fill = response)) +
  geom_bar() +
  labs(x = "Treatment", title = "Original") +
  theme(legend.position = "none") +
  coord_cartesian(ylim = c(0, height))
plot2 <- ggplot(BirdMalaria_shuffled, aes(x = shuffled_treatment, fill = response)) +
  geom_bar() +
...
shuffled_results <- BirdMalaria_shuffled
for (j in 1:n_shuffle) {
  shuffled_results <- BirdMalaria %>%  
...
```{r null-distribution-1, fig.cap="Distribution of shuffled differences in BirdMalaria.", purl=FALSE, echo=FALSE}
...
Again, notice how the `BirdMalaria` data itself doesn't change, but the `Response: response (factor)` and `Explanatory: treatment (factor)` *meta-data* do. This is similar to how the `group_by()` verb from `dplyr` doesn't change the data, but only adds "grouping" meta-data, as we saw in Section \@ref(groupby).
...
```{r echo=FALSE, purl=FALSE}
if (!file.exists("rds/BirdMalaria_generate.rds")) {
  BirdMalaria_generate <- BirdMalaria %>%
    specify(formula = response ~ treatment, success = "Malaria") %>%
    hypothesize(null = "independence") %>%
    generate(reps = 1000, type = "permute")
  write_rds(BirdMalaria_generate, "rds/BirdMalaria_generate.rds")
} else {
  BirdMalaria_generate <- read_rds("rds/BirdMalaria_generate.rds")
}
nrow(BirdMalaria_generate)
```
...
Observe that the resulting data frame has `r (total_birds*n_reps) %>% comma()` rows. This is because we performed shuffles/permutations for each of the `r total_birds` rows `r n_reps` times and $`r (total_birds*n_reps) %>% comma()` = `r n_reps` \cdot `r total_birds`$. If you explore the `BirdMalaria_generate` data frame with `View()`, you'll notice that the variable `replicate` indicates which resample each row belongs to. So it has the value `1` `r total_birds` times, the value `2` `r total_birds` times, all the way through to the value `r n_reps`, again `r total_birds` times. 
...
```{r echo=FALSE, purl=FALSE}
if (!file.exists("rds/null_distribution_BirdMalaria.rds")) {
  null_distribution <- BirdMalaria_generate %>%
    calculate(stat = "diff in props", order = c("Egg removal", "Control"))
  write_rds(null_distribution, "rds/null_distribution_BirdMalaria.rds")
} else {
  null_distribution <- read_rds("rds/null_distribution_BirdMalaria.rds")
}
null_distribution
```
...
```{r echo=FALSE, purl=FALSE}
if (!file.exists("rds/bootstrap_distribution_BirdMalaria.rds")) {
  bootstrap_distribution <- BirdMalaria %>%
    specify(formula = response ~ treatment, success = "Malaria") %>%
    # Change 1 - Remove hypothesize():
    # hypothesize(null = "independence") %>%
  ...
  write_rds(
    ...
    "rds/bootstrap_distribution_BirdMalaria.rds"
  )
} else {
  bootstrap_distribution <- read_rds("rds/bootstrap_distribution_BirdMalaria.rds")
}
```
...
```{r echo=FALSE, purl=FALSE}
# This code is used for dynamic non-static in-line text output purposes
n_HornedLizards <- HornedLizards %>% drop_na() %>% nrow()
```
...
Did you notice the warning above that one row contains "non-finite" values? Apparently one row has missing values -- which explains why the dataset has 185 rows instead of the expected 184 observations. Before we proceed further, let's use `drop_na` to drop the row with missing data to potentially avoid trouble later on.
