...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** Repeat the earlier installation steps, but for the `dplyr`, `openintro`, and `knitr` packages. This will install the earlier mentioned `dplyr` package for data wrangling, the `openintro` package contains the `mammals` data set with information on 39 mammalian species, and the `knitr` package for generating easy-to-read tables in R. We'll use these packages in the next section.
...
**`r paste0("(LC", chap, ".", (lc <- lc + 1), ")")`** "Load" the `dplyr`, `openintro`, `readr` and `knitr` packages as well by repeating the earlier steps.
